...
from agentforge.cli_quality import register as _register_quality
...
from agentforge.wiki_memory.typer_cli import register as _register_wiki
...
def forge(
    ...
    if target == "openclaw":
        console.print("[blue]Target:[/blue] OpenClaw")
    elif target == "personanexus":
        console.print("[blue]Target:[/blue] PersonaNexus deployment package")
        context["output_format"] = "personanexus"
    elif target == "plain":
        console.print("[blue]Target:[/blue] plain (analysis-only, decorative files suppressed)")
    if supplement:
...
def serve(
    ...
    Non-loopback binds (e.g. 0.0.0.0) require AGENTFORGE_API_TOKEN unless set
    to 'disabled'. Local 127.0.0.1 remains open when no token is configured.

    Examples:
        agentforge serve
        agentforge serve --port 8080 --reload
        AGENTFORGE_API_TOKEN=secret agentforge serve --host 0.0.0.0 --no-open
    """
    try:
        import uvicorn
    except ImportError:
    ...
    from agentforge.web.auth import auth_status, ensure_bind_auth

    try:
        ensure_bind_auth(host)
    except ValueError as e:
        console.print(Panel(str(e), title="Authentication Required", border_style="red"))
        raise typer.Exit(code=1)

    mode, _ = auth_status()
    auth_hint = {
        "required": "API auth: Bearer token required",
        "disabled": "API auth: explicitly disabled",
        "open": "API auth: open (loopback only)",
    }.get(mode, mode)
